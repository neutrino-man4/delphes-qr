...
nAttempt=3
mix=False
qr_cut_data_dir=f'/ceph/abal/CASE/QR_datasets/run_{run_n}/{nAttempt}'
# If smoothed
filename=os.path.join(qr_cut_data_dir,f'bkg+inj_{signal_name}_{siginj}.h5')    
...
labels = ['$q=0.7$-$1.0$', '$q=0.5$-$0.7$', '$q=0.3$-$0.5$', '$q=0.1$-$0.3$', '$q<0.05$']
...
DATA95,_ = np.histogram(hist_q05,bins=bins)#,histtype='step',label=labels[4])
...
yvals=[DATA,DATA30,DATA50,DATA70,DATA95]
...
ax.text(0.34,0.79,"$\Delta\eta_{jj} < 1.4$",fontsize=14,transform=ax.transAxes)
...
if siginj!=0:
    ...
    ax.text(0.34,0.73,"$M_{graviton}=3.5\,\\mathrm{TeV}$",fontsize=14,transform=ax.transAxes)
    ax.text(0.34,0.68,f"Injected signal: ${int(siginj*1000)} fb$",fontsize=14,transform=ax.transAxes)
if mix:
    ax.text(0.34,0.65,"With jet mixing",fontsize=11,transform=ax.transAxes)
else:
    ax.text(0.34,0.65,"Without jet mixing",fontsize=11,transform=ax.transAxes)
...
ax.text(0.34,0.88,f"QR applied on signal region QCD (Delphes dataset)",fontsize=14,transform=ax.transAxes,weight='bold')
...
ax.legend(frameon=False,fontsize=19,loc=(0.75,0.45),ncol=1)
#ax_ratio1.legend(loc='upper right')

# Create image folder if it doesn't exist
image_dir=f'/etpwww/web/abal/public_html/CASE/delphes_tests/{nAttempt}'
pathlib.Path(image_dir).mkdir(parents=True, exist_ok=True)

img_path = os.path.join(image_dir,f'{qcd_sample}.png')
if siginj!=0:
    img_path = os.path.join(image_dir,f'{qcd_sample}_inj_{siginj}_{signal_name}_q95.png')
