...
bkg_filename=f'/storage/9/abal/CASE/QR_results/events/run_{run_n}/sig_{signal_name}/xsec_0/loss_rk5_05/delphes_bkg.h5'
...
parser.add_argument("-f","--run-on-datalike",dest='force',default=False, action='store_true',help='Use if you want to run the QR on bkg+inj signal dataset irrespective')
...
for siginj in signal_injections:
    ...
    data_filename=f'/storage/9/abal/CASE/QR_results/events/run_{run_n}/sig_{signal_name}/xsec_0/loss_rk5_05/{inj_qcd_sample}.h5'
    if siginj==0.:
        case_qr_models = f'/work/abal/CASE/QR_models/run_{run_n}/delphes_models_lmfit_csv/{nAttempt}/{qcd_sample}'
    else:
    ...
    for k in tqdm.tqdm(range(folds)):
        ...
        for q in quantiles:
            if siginj == 0.:
                tmpdf = pd.read_csv(f'{case_qr_models}/bkg_lmfit_modelresult_fold_{k}_quant_q{q}.csv')
                sig_tmpdf = pd.read_csv(f'{case_qr_models}/sig_lmfit_modelresult_quant_q{q}.csv')
                ...
                tmpdf = pd.read_csv(f'{case_qr_models}/bkg+inj_lmfit_modelresult_fold_{k}_quant_q{q}.csv')
                sig_tmpdf = pd.read_csv(f'{case_qr_models}/sig_lmfit_modelresult_quant_q{q}.csv')
                sig_p = np.poly1d(sig_tmpdf['par'].values.tolist()[::-1])
                sig_qrs.append(sig_p)
    ...
    case_qr_datasets=f'/ceph/abal/CASE/QR_datasets/run_{run_n}/{nAttempt}'
    ...
    if siginj != 0:
        data_outfilename = 'bkg+inj_%s_%s.h5'%(signal_name,str(siginj))
        ...
        data_hf.create_dataset('eventFeatureNames', data=np.array(data_event_feature_names_reversed,dtype=dt))
        data_hf.close()
